...
- The optimal storage format for a `DataFrame` depends on the insight we are trying to glean from it.
  - We consider making `DataFrames` **wider** if we think one *observation* is spread across multiple *rows*.
  - We consider making `DataFrames` **longer** if we think one *variable* is spread across multiple *columns*.
...
- `melt()` can take a few parameters:
  ...
  - `id_vars` indicates which column should be the “identifier”.
...
- When using `pivot()`, we need to specify a few parameters:
  ...
  - `columns` that will be used to make the *new variables* of the wider `DataFrame`;
  - `values` that provide the existing column that defines the *values*.
