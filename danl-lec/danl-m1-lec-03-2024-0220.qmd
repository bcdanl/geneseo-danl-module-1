---
title: Lecture 2
subtitle: Pandas Basics - Sorting, Indexing, and Filtering Data with Missing Values and Duplicates
format:
  clean-revealjs:
    self-contained: true
    incremental: true
...
include-after-body: effect.html
...
- The above example compares each `First Name` value with "Maria".
...
`Series` to a descriptive object and then pass it into the square brackets instead.
...
What if we want to extract a subset of employees who are not on the "`Finance`" team?
...
- `True` denotes that the `Team` value for a given index is not "`Finance`", and `False` indicates the `Team` value is "`Finance`"
...
mktg = emp["Team"] == "Marketing"
emp[ sales | legal | mktg ]
```
...
on_all_star_teams = emp["Team"].isin(all_star_teams)
emp[on_all_star_teams]
```
...
- When working with numbers or dates, we often want to extract values that fall within a range. 
  - E.g., Identify all employees with a salary between $80,000 and $90,000. 
...
```{.python}
higher_than_80 = emp["Salary"] >= 80000
lower_than_90 = emp["Salary"] < 90000
emp[higher_than_80 & lower_than_90]
```
...
```{.python}
between_80k_and_90k = emp["Salary"].between(80000, 90000)
emp[between_80k_and_90k]
```

- A slightly cleaner solution is to use a method called `between()`
  - It accepts a lower bound and an upper bound; 
...
```{.python}
name_starts_with_r = emp["First Name"].between("R", "S")
emp[name_starts_with_r]
```
...
Let's do Questions 7-8 in Part 2 of [Classwork 3](https://bcdanl.github.io/module-1/danl-cw/danl-m1-cw-3.html)!
